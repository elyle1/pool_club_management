...
def display():
  ...
  treeframe = LabelFrame(root, text="Daily Report Preview")
  treeframe.pack(pady = 10)

  scrollbar = ttk.Scrollbar(treeframe)
  scrollbar.pack(side=RIGHT, fill=Y)

  dReport_tree = ttk.Treeview(treeframe, yscrollcommand=scrollbar.set, selectmode='extended')
  dReport_tree.pack()

  dReport_tree['columns'] = ("Preview")
  dReport_tree.column("#0", width=0, stretch=NO)
  dReport_tree.column("Preview", anchor=W, width=690)

  scrollbar.config(command=dReport_tree.yview)
